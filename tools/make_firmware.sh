...
FIRMWARE_PARTITION_HEADER_OFFSET=$( grep -boa "PARTfirmware" $OUTPUT_FIRMWARE_TEMP | grep -oP "^\d+" )
FIRMWARE_PARTITION_HEADER_SIZE=56
FIRMWARE_PARTITION_SIZE=$(printf "%d" 0x$(dd if=$OUTPUT_FIRMWARE_TEMP bs=1 skip=$((FIRMWARE_PARTITION_HEADER_OFFSET+48)) count=4 2>/dev/null | xxd -p))
